		...
		public function toAtlas(...args):JoinResult {
			...
			if (args.length == 1 && args[0] is Vector.<BitmapData>) {
				...
				if (bitmaps && bitmaps.length) {
					...
					var bitmapData:BitmapData = new BitmapData(rectangleSize.columns * bitmaps[0].width, rectangleSize.rows * bitmaps[0].height);
					...
					for (var i:uint = 0; i < length; ++i) {
						...
						regions.push(new Region("region-" + i, column * tmp.width, row * tmp.height, tmp.width, tmp.height));
